...
from .webshop-01-requests import get_request, get_webshop-01-settings, post_request, delete_request


def webshop-01-webhook(f):
	"""
	...
	def wrapper(*args, **kwargs):
		...
		# Verify the HMAC.
		if not _hmac_is_valid(frappe.local.request.get_data(), get_webshop-01-settings().password, webhook_hmac):
